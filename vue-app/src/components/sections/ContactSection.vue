<template>
  <section id="contact" class="scroll-mt-[100px] bg-secondary-color py-20 px-4 sm:px-6 lg:px-8">
    <div class="max-w-xl mx-auto">
      <h2 class="text-4xl font-bold text-center mb-12 reveal">Contact Us</h2>
      <div class="mb-8 text-center">
        <p class="text-lg text-text-secondary mb-4">Visit us or get in touch directly:</p>
        <p class="font-medium">Mr Riders Bike Rentals and Taxi Service</p>
        <p>NH 66, near Railway Station, opp. RNS Hospital</p>
        <p>Murdeshwar, Bhatkal, Karnataka 581350</p>
        <div class="flex justify-center gap-4 mt-4">
          <a href="tel:+919972544847" class="flex items-center text-primary-color">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
            </svg>
            +91 9972544847
          </a>
          <a href="https://wa.me/+919972544847" class="flex items-center text-green-600" target="_blank" rel="noopener noreferrer">
            <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 16 16">
              <path d="M13.601 2.326A7.854 7.854 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.933 7.933 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.898 7.898 0 0 0 13.6 2.326zM7.994 14.521a6.573 6.573 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.557 6.557 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592zm3.615-4.934c-.197-.099-1.17-.578-1.353-.646-.182-.065-.315-.099-.445.099-.132.197-.513.646-.627.775-.114.133-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.103-1.372-.114-.198-.011-.304.088-.403.087-.088.197-.232.296-.346.09-.14.12-.236.18-.355.058-.12.027-.221-.015-.308-.041-.087-.445-1.074-.612-1.47-.16-.389-.323-.335-.445-.34-.114-.007-.247-.007-.38-.007a.729.729 0 0 0-.529.247c-.182.198-.691.677-.691 1.654 0 .977.71 1.916.81 2.049.098.132 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.904.129 1.246.08.38-.051 1.171-.48 1.338-.943.164-.464.164-.86.114-.943-.049-.084-.182-.133-.38-.232z" />
            </svg>
            WhatsApp
          </a>
        </div>
      </div>
      <form class="space-y-6 reveal-scale" @submit.prevent="submitForm">
        <div>
          <label class="sr-only" for="name">Your Name</label>
          <input class="form-input w-full rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-primary-color border-gray-300 h-14 p-4 text-base" id="name" name="name" placeholder="Your Name" type="text" />
        </div>
        <div>
          <label class="sr-only" for="email">Your Email</label>
          <input class="form-input w-full rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-primary-color border-gray-300 h-14 p-4 text-base" id="email" name="email" placeholder="Your Email" type="email" />
        </div>
        <div>
          <label class="sr-only" for="message">Your Message</label>
          <textarea class="form-textarea w-full rounded-lg text-text-primary focus:outline-none focus:ring-2 focus:ring-primary-color border-gray-300 p-4 text-base" id="message" name="message" placeholder="Your Message" rows="6"></textarea>
        </div>
        <div class="text-center">
          <button class="min-w-[140px] rounded-lg h-12 px-6 bg-primary-color text-white text-lg font-bold tracking-wide hover:bg-opacity-90 transition-colors" type="submit">
            Send Message
          </button>
        </div>
      </form>
    </div>
  </section>
</template>

<script setup lang="ts">
const submitForm = () => {
  // Handle form submission
  alert('Form submitted!');
};
</script> 